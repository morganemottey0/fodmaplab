"use client";

import { signIn } from "next-auth/react";
import { useState } from "react";

export default function LoginPage() {
  const [loading, setLoading] = useState(false);

  const handleLogin = async () => {
    setLoading(true);
    await signIn("google", { callbackUrl: "/" });
  };

  return (
    <div className="fixed inset-0 bg-primary z-50 flex flex-col overflow-hidden">

      {/* Décorations */}
      <div className="decoration-circle-lg" />
      <div className="decoration-circle-sm" />

      {/* Haut */}
      <div className="flex-1 flex flex-col items-center justify-center px-6 pt-10">
        <div className="w-20 h-20 rounded-3xl flex items-center justify-center mb-6" style={{ background: "rgba(255,255,255,0.15)" }}>
          <svg width="40" height="40" viewBox="0 0 24 24" fill="none">
            <path d="M12 4V20M4 12H20" stroke="white" strokeWidth="2.5" strokeLinecap="round"/>
          </svg>
        </div>
        <h1 className="text-white text-4xl font-bold tracking-tight text-center mb-2">
          FODMAP AI
        </h1>
        <p className="text-center text-sm" style={{ color: "rgba(255,255,255,0.65)", lineHeight: 1.6 }}>
          Votre guide intelligent<br />pour le régime low-FODMAP
        </p>
      </div>

      {/* Card bas */}
      <div className="bg-surface rounded-t-3xl px-6 pt-8 pb-12">
        <h2 className="text-primary text-2xl font-semibold tracking-tight mb-1">
          Commencer
        </h2>
        <p className="text-muted text-sm mb-7" style={{ lineHeight: 1.6 }}>
          Connectez-vous pour suivre votre alimentation et accéder à votre espace personnel.
        </p>

        <button
          onClick={handleLogin}
          disabled={loading}
          className="btn-secondary flex items-center justify-center gap-3 mb-3"
        >
          <svg width="20" height="20" viewBox="0 0 24 24">
            <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
            <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
            <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
            <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
          </svg>
          {loading ? "Connexion..." : "Continuer avec Google"}
        </button>

        <button onClick={handleLogin} disabled={loading} className="btn-primary">
          Créer un compte
        </button>

        <p className="text-muted text-xs text-center mt-5" style={{ lineHeight: 1.6 }}>
          En continuant, vous acceptez nos conditions d'utilisation.
        </p>
      </div>
    </div>
  );
}